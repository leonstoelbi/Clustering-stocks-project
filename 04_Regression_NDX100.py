##### Program 4: Running a regression on the NASDAQ-100 data set to see what drives NASDAQ-100 stock prices
...
dataset = pd.read_csv("Dataset_complete_NDX100.csv", encoding="UTF-8")
...
def regress(ticker):
    ...
    reg_model = sm.ols(formula=ticker+" ~ NDX_Index + Oil + Gold + NatGas", data=dataset).fit()
...
def regress_test(ticker):
    result = sm.ols(formula=ticker+" ~ NDX_Index + Oil + Gold + NatGas", data=dataset).fit()
...
for ticker in dataset.columns[1:-5]:
    ...
    reg_matrix = pd.concat([reg_matrix,reg_stock], axis=1)
...
reg_matrix.to_csv("Reg_Matrix_NDX100.csv",encoding="UTF-8")