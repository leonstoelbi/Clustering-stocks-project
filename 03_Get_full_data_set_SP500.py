...
# Drop HPE from dataset since it is a new member of the SP500 and, therefore, causing anomalies (beta of 12 for the gold price) in the results
dataset.drop(["HPE"], axis = 1, inplace = True)
dataset.drop(["UA"], axis = 1, inplace = True)
print('HPE and UA are excluded since only recently added to S&P500\n-> would distort the regression in the next step')
