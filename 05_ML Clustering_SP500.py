...
reg_matrix = pd.read_csv("Reg_Matrix_SP500.csv",encoding="UTF-8")
...
stock_info = pd.read_csv("SP500_array.csv", encoding="UTF-8")
...
result = pd.DataFrame()
result = stock_info
result = result.groupby("Group")["GICS Sector"].value_counts()
# print(result)

## 6. Save the final result locally as a .csv file

result.to_csv("Result_SP500.csv",encoding="UTF-8")

stock_info.to_csv("Final_stock_array_SP500.csv",encoding="UTF-8")